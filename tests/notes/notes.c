	...
	const git_oid *blob_oid, const git_oid *annotated_obj_id, void *payload)
{
	...
	for (size_t i = 0; notes[i].note_oid != NULL; i++) {
...
static struct note_create_payload
...
void test_notes_notes__can_create_a_note(void)
{
	...
	for (size_t i = 0; can_create_a_note[i].note_oid != NULL; i++) {
		if (can_create_a_note[i].seen) {
			retrieved_notes++;
		}
	}
...
void test_notes_notes__can_create_a_note_from_commit(void)
{
	...
	for (size_t i = 0; can_create_a_note_from_commit[i].note_oid != NULL; i++) {
		if (can_create_a_note_from_commit[i].seen) {
			retrieved_notes++;
		}
	}
...
void test_notes_notes__can_create_a_note_from_commit_given_an_existing_commit(void)
{
	...
	for (size_t i = 0; can_create_a_note_from_commit_given_an_existing_commit[i].note_oid != NULL; i++) {
		if (can_create_a_note_from_commit_given_an_existing_commit[i].seen) {
			retrieved_notes++;
		}
	}
