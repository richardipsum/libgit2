	...
	int allow_note_overwrite)
{
	int error;
	char *target = NULL, *notes_ref = NULL;
	git_commit *commit = NULL, *notes_commit = NULL;
	git_reference *ref;
	const git_oid *notes_commit_oid;
	git_oid notes_blob_oid;
	...
	error = git_note_commit_create(&notes_commit,
			&notes_blob_oid,
			repo, commit, author,
			committer, oid, note,
	...
	notes_commit_oid = git_commit_id(notes_commit);
		...
		const git_oid *oid)
{
	...
	error = retrieve_note_commit(&commit, &notes_ref_target, repo, notes_ref_in);

	if (error < 0)
		goto cleanup;

	error = git_note_commit_remove(&notes_commit, repo,
			commit, author, committer, oid);
	if (error < 0)
		goto cleanup;

	error = git_reference_create(&notes_ref, repo, notes_ref_target,
			git_commit_id(notes_commit), 1, NULL);
